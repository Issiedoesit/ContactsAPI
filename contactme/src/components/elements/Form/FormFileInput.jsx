...
const FormFileInput = ({fieldsetId, handleChange, formik, handleCancel, setErrorUpload,  setUploadedImageUrl, handleFileUpload, uploadedImageUrl, actionText, fileValue, uploading, errorUpload, fileInputId, fileInputName}) => {
  ...
  return (
    <fieldset id={fieldsetId} className=''>
        <div className="mx-auto w-fit pb-4 text-slate-950">
          {
            fileValue
            ?
            <img src={fileValue ? isFileObject(fileValue) ? URL.createObjectURL(fileValue) : fileValue : 'https://res.cloudinary.com/issie/image/upload/v1690197188/memojis/female-memoji-2.png'} alt="Avatar" className='w-24 rounded-full aspect-square' />
            :
            ...
            // </svg>
            <img src="https://res.cloudinary.com/issie/image/upload/v1690197188/memojis/female-memoji-1.png" alt="Avatar" className={`w-24`} />
          }
        </div>
        <label htmlFor={fileInputId} className='w-fit cursor-pointer mx-auto flex flex-col gap-3 items-center justify-center font-bold'>
            <div className='text-xs rounded-full border-2 border-black py-2 px-4'>
            {
                !errorUpload
                ?
                <div>
                ...
                uploading ? 
                <span className={`flex gap-2 items-center text-slate-950`}>Uploading <BeatLoader size={'10px'} color={`#2A2AB2`} /></span> : 
                <span className={`text-slate-950`}>{actionText || 'Choose dp'}</span> : 
                <span className='flex gap-2 items-center text-slate-950'>
                    Logo Uploaded
                ...
                :
                <p className={`text-red-500 text-xs`}>Error uploading Logo</p>
            }
            </div>
        ...
        ?
          <p className={`text-xs text-right text-red-500 py-2`}>*** {formik.errors[fileInputId]}</p>
          :
          <div>
            {
              ...
              <div className={`flex items-center gap-3 justify-end py-2 flex-wrap`}>
                <p className='text-gray-400 text-xs'>Use this image ?</p>
                <div className={`flex items-center gap-3 justify-end`}>
                  <button type='button' disabled={uploading} onClick={handleCancel} className='text-red-500 disabled:text-gray-500 text-3xl'> <IoIosClose /></button>
                  <button type='button' disabled={uploading} onClick={handleFileUpload} className={`text-green-500 disabled:text-gray-500`}> <GiCheckMark /></button>
                </div>
              </div>
            }
          </div>
